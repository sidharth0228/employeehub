const Datastore = require('nedb-promises');
const path = require('path');
const bcrypt = require('bcryptjs');

const DATA_DIR = path.join(__dirname, 'data');

// Create datastores
const db = {
  users: Datastore.create({ filename: path.join(DATA_DIR, 'users.db'), autoload: true }),
  employees: Datastore.create({ filename: path.join(DATA_DIR, 'employees.db'), autoload: true }),
  documents: Datastore.create({ filename: path.join(DATA_DIR, 'documents.db'), autoload: true }),
  document_categories: Datastore.create({ filename: path.join(DATA_DIR, 'document_categories.db'), autoload: true }),
  leave_applications: Datastore.create({ filename: path.join(DATA_DIR, 'leave.db'), autoload: true }),
  payslips: Datastore.create({ filename: path.join(DATA_DIR, 'payslips.db'), autoload: true }),
  activity_log: Datastore.create({ filename: path.join(DATA_DIR, 'activity.db'), autoload: true }),
  settings: Datastore.create({ filename: path.join(DATA_DIR, 'settings.db'), autoload: true }),
};

// Helper to calculate salary breakdown from CTC
function calculateSalaryBreakdown(annualCtc) {
  const ctc = Number(annualCtc) || 600000;
  const monthlyGross = Math.round(ctc / 12);
  const basic = Math.round(monthlyGross * 0.5); // 50% basic
  const hra = Math.round(basic * 0.4); // 40% of basic
  const specialAllowance = Math.max(0, monthlyGross - basic - hra);
  const pfDeduction = Math.min(1800, Math.round(basic * 0.12)); // 12% of basic or standard PF
  const profTax = 200; // Standard PT
  const taxDeduction = ctc > 700000 ? Math.round((ctc - 700000) * 0.1 / 12) : 0;
  const totalDeductions = pfDeduction + profTax + taxDeduction;
  const inHandMonthly = monthlyGross - totalDeductions;

  return {
    ctc_annual: ctc,
    monthly_gross: monthlyGross,
    basic_salary: basic,
    hra: hra,
    special_allowance: specialAllowance,
    pf_deduction: pfDeduction,
    prof_tax: profTax,
    tax_deduction: taxDeduction,
    total_deductions: totalDeductions,
    in_hand_monthly: inHandMonthly,
    payment_mode: 'Bank Transfer'
  };
}

async function initDb() {
  const fs = require('fs');
  if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });

  const existingUser = await db.users.findOne({});
  if (!existingUser) {
    console.log('🌱 Seeding database...');

    // Seed admin
    const hash = bcrypt.hashSync('password123', 10);
    const admin = await db.users.insert({
      email: 'admin@employeehub.com',
      password_hash: hash,
      name: 'HR Admin',
      role: 'hr_admin',
      created_at: new Date().toISOString()
    });

    // Seed document categories
    const catData = [
      { name: 'Passport', is_required: true, description: 'Employee passport copy' },
      { name: 'Aadhaar Card', is_required: true, description: 'Government ID proof' },
      { name: 'PAN Card', is_required: true, description: 'Tax identification' },
      { name: 'Work Permit', is_required: false, description: 'Work authorization (for foreign nationals)' },
      { name: 'Educational Certificates', is_required: true, description: 'Degree / Diploma certificates' },
      { name: 'Experience Letters', is_required: false, description: 'Previous employment letters' },
      { name: 'Bank Statement', is_required: true, description: 'Recent bank statement for salary' },
      { name: 'Photo', is_required: true, description: 'Recent passport-size photograph' },
    ];
    const cats = [];
    for (const c of catData) {
      cats.push(await db.document_categories.insert({ ...c, created_at: new Date().toISOString() }));
    }

    // Seed employees with compensation
    const rawEmpData = [
      { employee_id: 'EH-001', first_name: 'Rahul', last_name: 'Sharma', email: 'rahul.sharma@company.com', phone: '+91-9876543210', department: 'Engineering', designation: 'Senior Software Engineer', employment_type: 'full_time', date_of_joining: '2022-03-15', date_of_birth: '1992-06-20', gender: 'male', nationality: 'Indian', status: 'active', onboarding_status: 'complete', onboarding_step: 4, address: '12, MG Road, Mumbai', bank_name: 'HDFC Bank', bank_account_number: '50100432198765', ifsc_code: 'HDFC0001234', ctc_annual: 1800000 },
      { employee_id: 'EH-002', first_name: 'Priya', last_name: 'Sharma', email: 'priya.sharma@company.com', phone: '+91-9876543211', department: 'HR', designation: 'HR Executive', employment_type: 'full_time', date_of_joining: '2023-01-10', date_of_birth: '1995-03-12', gender: 'female', nationality: 'Indian', status: 'active', onboarding_status: 'in_progress', onboarding_step: 3, address: '45, Park Street, Bangalore', bank_name: 'ICICI Bank', bank_account_number: '001205012345', ifsc_code: 'ICIC0000012', ctc_annual: 720000 },
      { employee_id: 'EH-003', first_name: 'Aman', last_name: 'Kumar', email: 'aman.kumar@company.com', phone: '+91-9876543212', department: 'Finance', designation: 'Finance Analyst', employment_type: 'full_time', date_of_joining: '2021-07-01', date_of_birth: '1990-11-05', gender: 'male', nationality: 'Indian', status: 'active', onboarding_status: 'complete', onboarding_step: 4, address: '78, Lake View, Hyderabad', bank_name: 'Axis Bank', bank_account_number: '918020034567890', ifsc_code: 'UTIB0000918', ctc_annual: 950000 },
      { employee_id: 'EH-004', first_name: 'Sarah', last_name: 'Jenkins', email: 'sarah.jenkins@company.com', phone: '+44-7700900123', department: 'Marketing', designation: 'Marketing Manager', employment_type: 'full_time', date_of_joining: '2023-06-01', date_of_birth: '1988-09-15', gender: 'female', nationality: 'British', status: 'active', onboarding_status: 'in_progress', onboarding_step: 2, address: '10 Downing Lane, London', bank_name: 'HSBC', bank_account_number: '40127654321098', ifsc_code: 'HSBC0560002', ctc_annual: 1600000 },
      { employee_id: 'EH-005', first_name: 'Michael', last_name: 'Chen', email: 'michael.chen@company.com', phone: '+1-4155550123', department: 'Engineering', designation: 'DevOps Engineer', employment_type: 'full_time', date_of_joining: '2022-09-20', date_of_birth: '1993-04-25', gender: 'male', nationality: 'American', status: 'active', onboarding_status: 'complete', onboarding_step: 4, address: '250 Bay Area Blvd, San Francisco', bank_name: 'Citi Bank', bank_account_number: '54321098765432', ifsc_code: 'CITI0000003', ctc_annual: 2100000 },
      { employee_id: 'EH-006', first_name: 'Emily', last_name: 'Davis', email: 'emily.davis@company.com', phone: '+91-9876543215', department: 'Design', designation: 'UI/UX Designer', employment_type: 'full_time', date_of_joining: '2022-11-15', date_of_birth: '1994-07-30', gender: 'female', nationality: 'Indian', status: 'on_leave', onboarding_status: 'complete', onboarding_step: 4, address: '34, Indiranagar, Bangalore', bank_name: 'HDFC Bank', bank_account_number: '50100789012345', ifsc_code: 'HDFC0001234', ctc_annual: 1250000 },
      { employee_id: 'EH-007', first_name: 'Vikram', last_name: 'Patel', email: 'vikram.patel@company.com', phone: '+91-9876543216', department: 'Engineering', designation: 'Backend Developer', employment_type: 'contract', date_of_joining: '2023-03-01', date_of_birth: '1991-12-10', gender: 'male', nationality: 'Indian', status: 'active', onboarding_status: 'pending', onboarding_step: 1, address: '90, Salt Lake, Kolkata', bank_name: 'State Bank of India', bank_account_number: '30456789012', ifsc_code: 'SBIN0001234', ctc_annual: 1400000 },
      { employee_id: 'EH-008', first_name: 'Neha', last_name: 'Singh', email: 'neha.singh@company.com', phone: '+91-9876543217', department: 'Operations', designation: 'Operations Lead', employment_type: 'full_time', date_of_joining: '2020-05-10', date_of_birth: '1987-02-18', gender: 'female', nationality: 'Indian', status: 'active', onboarding_status: 'complete', onboarding_step: 4, address: '67, Connaught Place, Delhi', bank_name: 'Kotak Mahindra', bank_account_number: '7812345678', ifsc_code: 'KKBK0000123', ctc_annual: 1100000 },
      { employee_id: 'EH-009', first_name: 'Raj', last_name: 'Mehta', email: 'raj.mehta@company.com', phone: '+91-9876543218', department: 'Sales', designation: 'Sales Executive', employment_type: 'full_time', date_of_joining: '2023-08-01', date_of_birth: '1996-05-22', gender: 'male', nationality: 'Indian', status: 'active', onboarding_status: 'in_progress', onboarding_step: 2, address: '23, Juhu Beach Road, Mumbai', bank_name: 'Axis Bank', bank_account_number: '919020087654321', ifsc_code: 'UTIB0000918', ctc_annual: 650000 },
      { employee_id: 'EH-010', first_name: 'Ananya', last_name: 'Roy', email: 'ananya.roy@company.com', phone: '+91-9876543219', department: 'Finance', designation: 'Accountant', employment_type: 'part_time', date_of_joining: '2022-02-14', date_of_birth: '1993-08-07', gender: 'female', nationality: 'Indian', status: 'active', onboarding_status: 'complete', onboarding_step: 4, address: '5, Park Circus, Kolkata', bank_name: 'HDFC Bank', bank_account_number: '50100987654321', ifsc_code: 'HDFC0001234', ctc_annual: 500000 },
      { employee_id: 'EH-011', first_name: 'Karan', last_name: 'Verma', email: 'karan.verma@company.com', phone: '+91-9876543220', department: 'Engineering', designation: 'QA Engineer', employment_type: 'full_time', date_of_joining: '2021-10-01', date_of_birth: '1992-01-30', gender: 'male', nationality: 'Indian', status: 'inactive', onboarding_status: 'complete', onboarding_step: 4, address: '11, Sector 18, Noida', bank_name: 'ICICI Bank', bank_account_number: '001205098765', ifsc_code: 'ICIC0000012', ctc_annual: 850000 },
      { employee_id: 'EH-012', first_name: 'Divya', last_name: 'Nair', email: 'divya.nair@company.com', phone: '+91-9876543221', department: 'Design', designation: 'Graphic Designer', employment_type: 'full_time', date_of_joining: '2023-04-15', date_of_birth: '1997-11-14', gender: 'female', nationality: 'Indian', status: 'active', onboarding_status: 'in_progress', onboarding_step: 3, address: '55, Koramangala, Bangalore', bank_name: 'HDFC Bank', bank_account_number: '50100456789012', ifsc_code: 'HDFC0001234', ctc_annual: 780000 },
      { employee_id: 'EH-013', first_name: 'Suresh', last_name: 'Babu', email: 'suresh.babu@company.com', phone: '+91-9876543222', department: 'Operations', designation: 'Operations Manager', employment_type: 'full_time', date_of_joining: '2019-12-01', date_of_birth: '1985-06-25', gender: 'male', nationality: 'Indian', status: 'active', onboarding_status: 'complete', onboarding_step: 4, address: '88, Anna Nagar, Chennai', bank_name: 'State Bank of India', bank_account_number: '30789012345', ifsc_code: 'SBIN0001234', ctc_annual: 1750000 },
      { employee_id: 'EH-014', first_name: 'Pooja', last_name: 'Gupta', email: 'pooja.gupta@company.com', phone: '+91-9876543223', department: 'Marketing', designation: 'Content Writer', employment_type: 'contract', date_of_joining: '2023-09-01', date_of_birth: '1998-03-09', gender: 'female', nationality: 'Indian', status: 'active', onboarding_status: 'pending', onboarding_step: 1, address: '101, Civil Lines, Jaipur', bank_name: 'Axis Bank', bank_account_number: '918020054321678', ifsc_code: 'UTIB0000918', ctc_annual: 550000 },
      { employee_id: 'EH-015', first_name: 'Arjun', last_name: 'Kapoor', email: 'arjun.kapoor@company.com', phone: '+91-9876543224', department: 'Sales', designation: 'Sales Manager', employment_type: 'full_time', date_of_joining: '2020-08-17', date_of_birth: '1988-10-03', gender: 'male', nationality: 'Indian', status: 'on_leave', onboarding_status: 'complete', onboarding_step: 4, address: '200, Bandra West, Mumbai', bank_name: 'HDFC Bank', bank_account_number: '50100345678901', ifsc_code: 'HDFC0001234', ctc_annual: 1950000 },
    ];

    const emps = [];
    for (const e of rawEmpData) {
      const breakdown = calculateSalaryBreakdown(e.ctc_annual);
      emps.push(await db.employees.insert({
        ...e,
        ...breakdown,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString()
      }));
    }

    // Seed documents
    const docData = [
      { employee_id: emps[0]._id, category_id: cats[0]._id, category_name: 'Passport', file_name: 'rahul_passport.pdf', expiry_date: '2024-07-15', status: 'verified', notes: '' },
      { employee_id: emps[0]._id, category_id: cats[1]._id, category_name: 'Aadhaar Card', file_name: 'rahul_aadhaar.pdf', expiry_date: null, status: 'verified', notes: '' },
      { employee_id: emps[0]._id, category_id: cats[2]._id, category_name: 'PAN Card', file_name: 'rahul_pan.pdf', expiry_date: null, status: 'pending', notes: 'Awaiting verification' },
      { employee_id: emps[2]._id, category_id: cats[3]._id, category_name: 'Work Permit', file_name: 'aman_work_permit.pdf', expiry_date: '2023-06-01', status: 'expired', notes: 'Work permit expired – renewal required' },
      { employee_id: emps[1]._id, category_id: cats[2]._id, category_name: 'PAN Card', file_name: 'priya_pan.pdf', expiry_date: null, status: 'rejected', notes: 'Image unclear, please resubmit' },
      { employee_id: emps[3]._id, category_id: cats[0]._id, category_name: 'Passport', file_name: 'sarah_passport.pdf', expiry_date: '2027-03-20', status: 'verified', notes: '' },
      { employee_id: emps[4]._id, category_id: cats[1]._id, category_name: 'Aadhaar Card', file_name: 'michael_id.pdf', expiry_date: null, status: 'verified', notes: '' },
    ];
    for (const d of docData) {
      await db.documents.insert({ ...d, file_url: null, mime_type: null, uploaded_at: new Date().toISOString() });
    }

    // Seed leave applications with detailed reasons
    const leaveData = [
      { employee_id: emps[3]._id, employee_name: 'Sarah Jenkins', emp_code: 'EH-004', department: 'Marketing', leave_type: 'Sick Leave', start_date: '2026-08-22', end_date: '2026-08-23', days_count: 2, reason: 'Viral fever and severe migraine, doctor recommended 2 days rest', status: 'pending' },
      { employee_id: emps[4]._id, employee_name: 'Michael Chen', emp_code: 'EH-005', department: 'Engineering', leave_type: 'Annual Leave', start_date: '2026-08-25', end_date: '2026-08-30', days_count: 5, reason: 'Annual family vacation trip to Hawaii', status: 'approved' },
      { employee_id: emps[5]._id, employee_name: 'Emily Davis', emp_code: 'EH-006', department: 'Design', leave_type: 'Maternity Leave', start_date: '2026-08-20', end_date: '2026-11-20', days_count: 90, reason: 'Maternity leave as per company policy and doctor certificate', status: 'pending' },
      { employee_id: emps[0]._id, employee_name: 'Rahul Sharma', emp_code: 'EH-001', department: 'Engineering', leave_type: 'Casual Leave', start_date: '2026-08-24', end_date: '2026-08-24', days_count: 1, reason: 'Personal family commitment and bank documentation work', status: 'pending' },
      { employee_id: emps[1]._id, employee_name: 'Priya Sharma', emp_code: 'EH-002', department: 'HR', leave_type: 'Sick Leave', start_date: '2026-08-15', end_date: '2026-08-16', days_count: 2, reason: 'Dental surgery and post-op recovery', status: 'approved' },
      { employee_id: emps[14]._id, employee_name: 'Arjun Kapoor', emp_code: 'EH-015', department: 'Sales', leave_type: 'Annual Leave', start_date: '2026-08-18', end_date: '2026-08-25', days_count: 7, reason: 'Attending sibling wedding ceremony in Udaipur', status: 'approved' },
      { employee_id: emps[2]._id, employee_name: 'Aman Kumar', emp_code: 'EH-003', department: 'Finance', leave_type: 'Casual Leave', start_date: '2026-08-10', end_date: '2026-08-10', days_count: 1, reason: 'Personal errands', status: 'rejected' },
    ];
    for (const l of leaveData) {
      await db.leave_applications.insert({ ...l, applied_at: new Date().toISOString(), reviewed_by: null, reviewed_at: null, notes: '' });
    }

    // Seed payslips for August 2026 & July 2026
    const months = ['August 2026', 'July 2026'];
    for (const m of months) {
      for (const emp of emps) {
        const isAug = m === 'August 2026';
        await db.payslips.insert({
          employee_id: emp._id,
          employee_name: `${emp.first_name} ${emp.last_name}`,
          emp_code: emp.employee_id,
          department: emp.department,
          designation: emp.designation,
          month_year: m,
          pay_period: isAug ? 'Aug 01, 2026 - Aug 31, 2026' : 'Jul 01, 2026 - Jul 31, 2026',
          ctc_annual: emp.ctc_annual,
          monthly_gross: emp.monthly_gross,
          basic_salary: emp.basic_salary,
          hra: emp.hra,
          special_allowance: emp.special_allowance,
          pf_deduction: emp.pf_deduction,
          prof_tax: emp.prof_tax || 200,
          tax_deduction: emp.tax_deduction || 0,
          total_deductions: emp.total_deductions,
          in_hand_monthly: emp.in_hand_monthly,
          payment_mode: emp.payment_mode || 'Bank Transfer',
          bank_name: emp.bank_name || 'HDFC Bank',
          bank_account_number: emp.bank_account_number || '50100432198765',
          ifsc_code: emp.ifsc_code || 'HDFC0001234',
          status: isAug ? (emp.status === 'active' ? 'paid' : 'pending') : 'paid',
          disbursed_at: isAug ? new Date().toISOString() : new Date(Date.now() - 30 * 86400000).toISOString(),
          created_at: new Date().toISOString()
        });
      }
    }

    // Seed settings
    const settingsData = [
      { key: 'org_name', value: 'EmployeeHub Inc.' },
      { key: 'org_email', value: 'hr@employeehub.com' },
      { key: 'org_phone', value: '+91-1234567890' },
      { key: 'org_address', value: '123 Business Park, Mumbai, India' },
      { key: 'org_website', value: 'https://employeehub.com' },
      { key: 'leave_annual_quota', value: '21' },
      { key: 'leave_sick_quota', value: '10' },
      { key: 'leave_casual_quota', value: '7' },
    ];
    for (const s of settingsData) {
      await db.settings.insert(s);
    }

    // Seed activity log
    await db.activity_log.insert({ actor_id: admin._id, actor_name: 'HR Admin', action: 'verified', entity_type: 'employee', entity_id: emps[2]._id, details: 'Aman Kumar verified by HR Admin', created_at: new Date(Date.now() - 7200000).toISOString() });
    await db.activity_log.insert({ actor_id: admin._id, actor_name: 'HR Admin', action: 'added', entity_type: 'employee', entity_id: emps[3]._id, details: 'New employee Sarah Jenkins added with CTC ₹16,00,000', created_at: new Date(Date.now() - 18000000).toISOString() });
    await db.activity_log.insert({ actor_id: admin._id, actor_name: 'HR Admin', action: 'approved', entity_type: 'leave', entity_id: null, details: 'Leave approved for Michael Chen (Annual Vacation)', created_at: new Date(Date.now() - 86400000).toISOString() });

    console.log('✅ Database seeded successfully with payroll & compensation data');
  } else {
    // Migration: ensure existing employee records have salary breakdown
    const emps = await db.employees.find({});
    for (const e of emps) {
      if (!e.ctc_annual || !e.in_hand_monthly) {
        const defaultCtc = e.designation?.includes('Senior') || e.designation?.includes('Manager') ? 1800000 : 850000;
        const breakdown = calculateSalaryBreakdown(defaultCtc);
        await db.employees.update({ _id: e._id }, { $set: breakdown });
      }
    }

    // Migration: ensure initial payslips exist
    const payslipCount = await db.payslips.count({});
    if (payslipCount === 0) {
      for (const emp of emps) {
        const breakdown = calculateSalaryBreakdown(emp.ctc_annual || 850000);
        await db.payslips.insert({
          employee_id: emp._id,
          employee_name: `${emp.first_name} ${emp.last_name}`,
          emp_code: emp.employee_id,
          department: emp.department,
          designation: emp.designation,
          month_year: 'August 2026',
          pay_period: 'Aug 01, 2026 - Aug 31, 2026',
          ...breakdown,
          bank_name: emp.bank_name || 'HDFC Bank',
          bank_account_number: emp.bank_account_number || '50100432198765',
          ifsc_code: emp.ifsc_code || 'HDFC0001234',
          status: emp.status === 'active' ? 'paid' : 'pending',
          disbursed_at: new Date().toISOString(),
          created_at: new Date().toISOString()
        });
      }
    }
  }
}

module.exports = { db, initDb, calculateSalaryBreakdown };
